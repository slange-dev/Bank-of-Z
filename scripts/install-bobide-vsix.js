...
/**
 * Install Bob IDE VSIX Script
 * 
 * This script automates installing .vsix files into Bob IDE using the `bobide` command.
 ...
 * 
 * Usage:
 *   node scripts/install-bobide-vsix.js [vsix-directory]
 * 
 * Examples:
 *   node scripts/install-bobide-vsix.js
 *   node scripts/install-bobide-vsix.js ./vsix-extensions
 *   node scripts/install-bobide-vsix.js /path/to/custom/directory
 */
...
function installVsix(vsixPath) {
  return new Promise((resolve) => {
    ...
    const process = spawn('bobide', ['--install-extension', vsixPath], {
      stdio: 'pipe'
    });
 ...
 */
async function main() {
  console.log('Bob IDE Extension Installer\n');
  console.log('='.repeat(60));
  
  // Check if bobide command is available
  ...
  if (failCount > 0) {
    ...
    console.log('To reload: Restart Bob IDE or use the reload command if available.\n');
  }
}
